...
const parseDesiredState = (text) => {
    ...
    const put = (key, value) => {
        if (typeof key !== "string" || key.length === 0)
            throw new ActionError("Invalid key found.");
        if (typeof value !== "string")
            throw new ActionError(`Value for key '${key}' must be a string.`);
        if (map.has(key))
            throw new ActionError(`Duplicate key found in source file: ${key}`);
        map.set(key, value);
    };
    if ("data" in parsed) {
        const data = parsed.data;
        if (!Array.isArray(data))
            throw new ActionError("Canonical format requires 'data' array.");
...
const createUpdateBatches = (puts, deletes) => {
    const batches = [];
    let putIndex = 0;
    let deleteIndex = 0;
...
const awsCliJson = async (operation, payload, region) => {
...
const listAllKeys = async (kvsArn, region) => {
...
const applySingleBatchWithRetry = async (kvsArn, puts, deletes, region, retries = 3) => {
...
const updateKeysWithRetry = async (kvsArn, puts, deletes, region, retries = 3) => {
